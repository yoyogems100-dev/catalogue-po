          ...
          {categories.map((cat) => (
            <li key={cat.id}>
              <Link href={cat.href} className={s.catTile}>
                <div className={s.catImg}>{cat.image ? <img src={cat.image.src} srcSet={cat.image.srcSet} sizes="(min-width: 900px) 25vw, 50vw" alt={cat.image.alt} loading="lazy" /> : <div className={s.catImgEmpty} />}</div>
