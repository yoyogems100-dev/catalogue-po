...
type Options = Record<'site_category' | 'color' | 'shape' | 'size', Option[]>;

const TYPE_LABEL: Record<keyof Options, string> = { site_category: 'Category gallery', color: 'Colour', shape: 'Shape', size: 'Size' };
...
export default function MediaLibrary({ initial, links: initialLinks, options }: { initial: MediaRow[]; links: Link[]; options: Options }) {
  ...
  const shown = useMemo(() => {
    ...
    return items.filter((m) => {
      if (filter === 'unused' && links.some((l) => l.media_id === m.id)) return false;
      ...
      <div className={s.mediaGrid}>
        {shown.map((m) => (
          <button key={m.id} type="button" className={s.mediaCard} onClick={() => setOpen(m)}>
            ...
            <span>{usage(m.id).length ? `Used in ${usage(m.id).length}` : 'Not used'}{m.tags?.length ? ` · ${m.tags.join(', ')}` : ''}</span>
          </button>
        ))}
      </div>
...
function MediaDetail({ media, links: initialLinks, options, onClose, onSaved, onDeleted }: {
  ...
  async function remove() {
    const where = links.length ? ` It is used in ${links.length} place${links.length > 1 ? 's' : ''} and will disappear from ${links.length > 1 ? 'them' : 'it'}.` : '';
    if (!confirm(`Delete this image permanently?${where}`)) return;
      ...
      <div className={s.modal} role="dialog" aria-modal="true" aria-label="Image details" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={s.detail} style={{ overflow: 'auto', paddingTop: 10 }}>
          ...
          <div>
            ...
            <div className={s.field}>
              <span className={s.label}>Used for</span>
