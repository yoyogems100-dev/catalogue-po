...
type Supplier = { id: number; name: string; contact_name?: string | null; phone?: string | null; email?: string | null; notes?: string | null };
type Category = { id: number; name: string };
type Rate = { id: number; categoryId: number; categoryName: string; shapeName: string; sizeMm: string; colorName: string; costPrice: number; currency: string; notes: string | null };
...
export default function SupplierManager({ supplier, categories, linkedCategoryIds, rates }: { supplier: Supplier; categories: Category[]; linkedCategoryIds: number[]; rates: Rate[] }) {
  ...
  const [form, setForm] = useState({ name: supplier.name, contactName: supplier.contact_name || '', phone: supplier.phone || '', email: supplier.email || '', notes: supplier.notes || '' });
  ...
  const [costPrice, setCostPrice] = useState(''); const [currency, setCurrency] = useState('INR'); const [rateNotes, setRateNotes] = useState('');
  ...
  async function addRate() {
    ...
    setSaving(true); setMessage('');
    const response = await fetch(`/api/admin/suppliers/${supplier.id}/rates`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ categoryId, shapeId: shapeId || null, sizeId: sizeId || null, colorId: colorId || null, costPrice, currency, notes: rateNotes }) });
    const data = await response.json().catch(() => ({})); setSaving(false);
  ...
  return <>
    <section className="card admin-profile-editor"><h2>Supplier details</h2><div className="admin-profile-grid">
      <label>Supplier name<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Contact person<input value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} /></label><label>Phone<input value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label><label>Email<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label className="admin-profile-wide">Notes<textarea rows={3} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label>
