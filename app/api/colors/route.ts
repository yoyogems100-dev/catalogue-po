...
export async function PATCH(req: NextRequest) {
  const { id, hex_value } = await req.json();
  if (!id || !hex_value) return NextResponse.json({ error: 'id and hex_value required' }, { status: 400 });
  const { error } = await supabaseAdmin.from('colors').update({ hex_value }).eq('id', id);
