...
export async function GET(_req:NextRequest,{params}:{params:Promise<{id:string}>}){
 const includePrices=_req.nextUrl.searchParams.get('type')==='prices';
 ...
 const {data:colorLinks}=includePrices?await supabasePublic.from('category_colors').select('color_id').eq('category_id',id):{data:null};
 const colorId=colorLinks?.[0]?.color_id;
 const sections:SizeChartSection[]=await Promise.all((shapes.data||[]).map(async(s:any)=>{
  ...
  return {name:s.shapes.name,image,rows:(sizes.data||[]).filter((r:any)=>r.shape_sizes?.shape_id===s.shape_id).map((r:any)=>({size:r.shape_sizes.size_mm,diamondEquivalentCt:r.diamond_equivalent_ct===null?null:Number(r.diamond_equivalent_ct),...(includePrices?{priceInr:pricing&&colorId?lineInrPrice(pricing,s.shape_id,r.shape_sizes.id,colorId):null}:{})})).sort((a,b)=>compareDimensions(a.size,b.size))};
 }));
