...
import { hotKey, isHot, type HotFlags, type OptionKind } from '@/lib/hot-selling';
const HotContext = createContext({ flags: {} as HotFlags, ready: false, error: '', toggle: async (_categoryId: number, _kind: OptionKind, _id: number) => {} });
...
export function HotSellingProvider({children}: {children: React.ReactNode}) {
  ...
  async function toggle(categoryId:number,kind: OptionKind,id: number) {
    const key = hotKey(categoryId,kind,id);
    if (!ready || pending.current.has(key)) return;
    pending.current.add(key);
    setError('');
    const enabled = !flags[key];
    try {
      const result = await fetch('/api/admin/hot-selling', {method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({categoryId,kind,id,enabled})});
...
export function HotMark({categoryId,kind,ids,name}: {categoryId?:number;kind?:OptionKind;ids:number[];name:string}) {
  const {flags,ready,toggle} = useHotSelling();
  const editing = useContext(EditingContext);
  const [busy,setBusy] = useState(false);
  const hot = isHot(flags,categoryId,kind,ids);
  if (!categoryId || !kind) return null;
  if (editing && ids.length === 1) return <button type="button" className="hot-toggle" disabled={!ready || busy} aria-pressed={hot} aria-label={`${hot?'Remove':'Mark'} ${name} ${hot?'from':'as'} hot selling`} title={hot?'Remove hot selling':'Mark as hot selling'} onKeyDown={e => e.stopPropagation()} onClick={async e => {e.stopPropagation();setBusy(true);try {await toggle(categoryId,kind,ids[0]);}finally {setBusy(false);}}}> <span aria-hidden="true" style={{filter:hot?'none':'grayscale(1)',opacity:hot?1:0.45}}>🔥</span></button>;
